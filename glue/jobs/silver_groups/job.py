...
df = (
    df.withColumn("group_id",        F.col("group_id").cast("bigint"))
      .withColumn("parent_group_id", F.col("parent_group_id").cast("bigint"))
...
# ===============================================================
# 6.1 DEDUPLICAÇÃO — 1 GROUP POR group_id
# ===============================================================
...
df = (
    df.withColumn("rn", F.row_number().over(w_group))
      .filter(F.col("rn") == 1)
      .drop("rn")
)
...
print(
    f"### silver_groups FINALIZADO — MANTIDA APENAS p_source_date={latest_date} ###"
)
