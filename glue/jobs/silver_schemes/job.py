...
df = df.withColumn("scheme_id", F.col("scheme_id").cast("bigint"))
...
# ===============================================================
# 6.1 FILTRO DE CHAVE NULA — scheme_id
# ===============================================================

df = df.filter(F.col("scheme_id").isNotNull())

# ===============================================================
# 6.2 DEDUPLICAÇÃO — 1 SCHEME POR scheme_id
# ===============================================================

w_scheme = (
    Window
        .partitionBy("scheme_id")
        .orderBy(F.col("p_source_date").desc())
)

df = (
    df.withColumn("rn", F.row_number().over(w_scheme))
      .filter(F.col("rn") == 1)
      .drop("rn")
)

# ===============================================================
# 6.3 IDENTIFICA DATA MAIS RECENTE
# ===============================================================
...
# ===============================================================
# 6.4 LIMPA PARTIÇÕES ANTIGAS NO S3
# ===============================================================
        ...
        TableInput={
            ...
            "StorageDescriptor": {
                "Columns": [
                    {
                        "Name": f.name,
                        "Type": glue_type(f.dataType.simpleString()),
                    }
...
print(
    f"### silver_schemes FINALIZADO — MANTIDA APENAS p_source_date={latest_date} ###"
)
