...
def delete_s3_prefix(s3_uri):
    ...
    for page in paginator.paginate(
        Bucket=p.netloc, Prefix=p.path.lstrip("/")
    ):
...
# ===============================================================
# 6. p_source_date (DATA DO ARQUIVO OU DATA DE INGESTÃO)
# ===============================================================
...
# ===============================================================
# 6.1 DEDUPLICAÇÃO — 1 DRIVER POR driver_id
# ===============================================================
...
df = (
    df.withColumn("rn", F.row_number().over(w_driver))
      .filter(F.col("rn") == 1)
      .drop("rn")
)
...
print(
    f"### SILVER DRIVERS FINALIZADO — MANTIDA APENAS p_source_date={latest_date} ###"
)
